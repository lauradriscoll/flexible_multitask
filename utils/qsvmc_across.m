function [accuracy_test, accuracy_train] = ...
    qsvmc_across(data_vec,c1_vec,u_vec,g_vec,C)
...
accuracy_test = nan(size(data_vec,1));
accuracy_train = nan(size(data_vec,1));

for train_ind = 1:size(data_vec,1)
    for test_ind = 1:size(data_vec,1)
        data_vec_use_train = data_vec(train_ind,u_vec,g_vec);
        data_vec_use_test = data_vec(test_ind,u_vec,g_vec);
        c1_vec_use = c1_vec(g_vec);

        train_inds = randperm(size(data_vec_use_train,3),round(.75*size(data_vec_use_train,3)));
        test_inds = find(~ismember(1:size(data_vec_use_train,3),train_inds));

        if size(train_inds,2)>10 % require at least 10 observations

            % create training and testing subsets
            x_test = data_vec_use_test(:,test_inds)';
            y_test = c1_vec_use(test_inds);

            x_train = data_vec_use_train(:,train_inds)';
            ...
            accuracy_train(train_ind,test_ind) = nansum(train_pred==y_train)/size(y_train,1);
            test_pred = predict(svm_struct,x_test);
            accuracy_test(train_ind,test_ind) = nansum(test_pred==y_test)/size(y_test,1);
        end
    end
end
