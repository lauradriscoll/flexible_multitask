...
def _compute_variance_bymodel(model, sess, rules=None, random_rotation=False):
    ...
    for rule in rules:
        trial = generate_trials(rule, hp, 'test', noise_on=False, batch_size = 1024)
        ...
        for e_name, e_time in trial.epochs.items():
            if 'fix' not in e_name:  # Ignore fixation period 11032021 #remove fix 060821
    ...
    for data_type in ['rule', 'epoch']:
        ...
        h_var_end = np.zeros((n_hidden, len(h_all.keys())))
        for i, val in enumerate(h_all.values()):
            ...
            val_reshape = np.reshape(val,(-1,n_hidden))
            h_var_all[:, i] = val_reshape.var(axis = 0)#LND# val.var(axis=1).mean(axis=0)#GRY#
            val_end = val[-1,:,:]
            h_var_end[:, i] = val_end.var(axis = 0)#LND# val.var(axis=1).mean(axis=0)#GRY#

        result = {'h_var_all': h_var_all, 'h_var_end': h_var_end, 'keys': list(h_all.keys())}
